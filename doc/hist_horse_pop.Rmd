...
The R[@R] programming language and the following R packages were used to perform the analysis: knitr[@knitr] and tidyverse[@tidyverse]. The code used to perform the analysis and create this report can be found here: https://github.com/ttimbers/equine_numbers_value_canada_rmd.


## Results

```{r plot horses, fig.width=10, fig.height=3, fig.cap="Figure 1. Changes in horse populations in different provinces in Canada through time"}
...
# horse_plot
knitr::include_graphics('hist_horse_pop_files/figure-gfm/plot horses-1.png')
```
Figure 1. Changes in horse populations in different provinces in Canada from 1940-1072
...
knitr::kable(max_horses, caption = 'Table 1. Range of horse population (by province) in Canada')
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Figure 2. Horse population over th years in the province of Alberta", out.width='100%', fig.align='center'}
...
province_plot
```
Figure 2. Horse population over the years in the province of `r params$province`
